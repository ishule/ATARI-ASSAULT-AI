...
int main() {
    ...
    cout << "Acc train (OR): " << accuracy(p_or, X_or, Y_or) << "%\n";
    for (size_t i=0;i<X_or.size();++i) {
        auto out = p_or.predict(X_or[i]);
        double pred = out[0] >= 0.0 ? 1.0 : -1.0;
        cout << "  In: ("<<X_or[i][0]<<","<<X_or[i][1]<<") -> "<< pred << " (raw="<< out[0] <<")\n";
    }
    ...
    cout << "Acc train (XOR): " << accuracy(p_xor, X_xor, Y_xor) << "%\n";
    for (size_t i=0;i<X_xor.size();++i) {
        auto out = p_xor.predict(X_xor[i]);
        double pred = out[0] >= 0.0 ? 1.0 : -1.0;
        cout << "  In: ("<<X_xor[i][0]<<","<<X_xor[i][1]<<") -> "<< pred << " (raw="<< out[0] <<")\n";
    }
