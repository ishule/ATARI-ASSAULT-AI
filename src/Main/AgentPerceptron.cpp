...
int main() {
    ...
    try {
        ...
        cout << ">>> CEREBRO CARGADO OK.\n";
    } catch (...) {
        cerr << "ERROR: No encuentro 'models/assault_perceptron.txt'.\n";
        return 1;
    }
    ...
    while (!ale.game_over()) {
        ...
        if (ale.getEpisodeFrameNumber() % 20 == 0) {
            cout << " R:" << fixed << setprecision(2) << valRight 
                      << " L:" << valLeft 
                      << " F:" << valFire << " | "
                      << setw(10) << actionToString(action) << " | "
                      << (int)totalScore // Imprimimos la variable acumulada
                      << "\r" << flush;
        }
    }
    
    cout << "\nFIN. Score Real: " << totalScore << endl;
    return 0;
}